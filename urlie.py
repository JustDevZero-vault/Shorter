...
#------------------------------ Utilities --------------------------------------- #
# The main purpose of this script is to easily give urls with urlie service.
...
def splitter(responde):
    try:
        prog = compile('<input id="txtMain".*?/>')
        line=search(prog, responde).group()
        cleaned=line.replace('<input id="txtMain" name="url" style="width: 300px;" class="txtInput large" type="text" readonly onclick="this.select();" value="', '')
        cleaned=cleaned.replace('" tabindex="1" />','')
        return cleaned
...
def shorten_url(long_url):
    if check_url(long_url):
        try:
            version=urlencode(dict(ver="14"))
            longUrl=urlencode(dict(url=long_url))
            encodeurl='http://url.ie/site/create/?&%s&%s' % (version, longUrl)
